...
def main(model_name, param):
    ...
    if not os.path.exists(model_location):
        ...
        x = Conv2D(filters=20, kernel_size=(5, 5), strides=1, padding='valid', kernel_initializer=initialiser_normal,
                   bias_initializer=initialiser_const, activation='relu')(image_input)
        x = MaxPool2D(strides=2, pool_size=2)(x)
        x = Conv2D(filters=50, kernel_size=(5, 5), strides=1, padding='valid', kernel_initializer=initialiser_normal,
                   bias_initializer=initialiser_const, activation='relu')(x)
        x = MaxPool2D(strides=2, pool_size=2)(x)
        x = Flatten()(x)
        ...
        model.compile(optimizer=tf.keras.optimizers.SGD(), loss=angle_loss_fn, metrics=[angle_loss_fn])
        y_gaze_train = training_data_operators.operator_change_labels(y_gaze_train,
                                                                      props.change_label['change_label_label'],
                                                                      param)  # properties.change_label['change_label_pct'])
...
if __name__ == '__main__':
    ...
    param = [3.12, 6.25, 9.38, 12.5, 18.75]

    for p in param:
        for i in range(200):
            print("Param {}, Model {}".format(p, i))
            mn = 'lenet_change_label_mutated0_MP_{}_{}.h5'.format(p, i)
