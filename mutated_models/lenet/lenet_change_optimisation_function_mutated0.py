...
def main(model_name):
    ...
    if (not os.path.exists(model_location)):
        ...
        model.compile(optimizer=optimiser_operators.operator_change_optimisation_function(tf.keras.optimizers.SGD()), loss=angle_loss_fn, metrics=[angle_loss_fn])
...
if (__name__ == '__main__'):
    ...
    params = getattr(props, 'change_optimisation_function')
    param = ['adadelta', 'adagrad', 'adam', 'rmsprop', 'adamax']

    for p in param:
      params['optimisation_function_udp'] = p
      for i in range(200):
          print("Param {}, Model {}".format(p, i)) 
          mn = 'lenet_change_optimisation_function_mutated0_MP_{}_{}.h5'.format(p, i) 
