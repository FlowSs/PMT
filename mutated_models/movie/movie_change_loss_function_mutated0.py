...
def main(model_name):
    ...
    model.compile(loss=loss_operators.operator_change_loss_function(tf.keras.losses.BinaryCrossentropy()), optimizer=keras.optimizers.Adam(lr=0.001), metrics=[tf.keras.losses.BinaryCrossentropy()])
...
if (__name__ == '__main__'):
    ...
    param = ['mean_squared_error', 'mean_absolute_error', 'huber_loss', 'squared_hinge', 'categorical_crossentropy']
    
    for p in param:
     params["loss_function_udp"] = p
     for i in range(200):
          print("Param {}, Model {}".format(p, i))
          mn = 'movie_recomm_change_loss_function_mutated0_MP_{}_{}'.format(p, i)
