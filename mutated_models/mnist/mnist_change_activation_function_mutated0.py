...
def main(model_name):
    ...
    if not os.path.exists(model_location):
        ...
        model.save(model_location) #os.path.join('trained_models', 'mnist_trained.h5'))
...
if (__name__ == '__main__'):
    ...
    params = getattr(props, 'change_activation_function')
    param = ['elu', 'softmax', 'tanh', 'sigmoid', 'exponential']
    # Layer number, chose layer 6 to match the pretrained model of DeepCrime
    params["current_index"] = 6

    for p in param:
      params["activation_function_udp"] = p
      #try:
      for i in range(200):
          print("Param {}, Model {}".format(p,i))
          mn = 'mnist_change_activation_function_mutated0_MP_{}_{}.h5'.format(p, i)
